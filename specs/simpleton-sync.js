...
//     call abort() to cancel the subscription.
//
...
function simpleton_client(url, {
    ...
    braid_fetch(url, {
        ...
        res.subscribe(async update => {
            ...
            update.parents.sort()
            if (client_version.length === update.parents.length
                && client_version.every((v, i) => v === update.parents[i])) {
                ...
                client_version = update.version.sort()
    ...
    return {
      ...
      // 1. Checks the throttle (outstanding_changes >= max)
      // 2. Diffs client_state vs current state
      ...
      changed: () => {
        ...
        if (generate_local_diff_update) {
            ...
            if (new_state === client_state) return // Stop if there wasn't a change!
        ...
        // ── Async accumulation loop ─────────────────────────────
        // This loop sends the current diff, then checks if more
        // edits accumulated while the PUT was in flight. If so, it
        // diffs and sends again. Equivalent to "flush on PUT ACK".
        ;(async () => {
            while (true) {
                ...
                if (generate_local_diff_update) {
                    update = generate_local_diff_update(client_state)
                    if (!update) return
                    ;({patches, new_state} = update)
                } else {
                    new_state = get_state()
                    if (new_state === client_state) return
                    patches = get_patches ? get_patches(client_state) :
                        [simple_diff(client_state, new_state)]
                }
            }
        })()
